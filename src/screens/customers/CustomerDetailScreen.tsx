...
export default function CustomerDetailScreen({ navigation, route }: any) {
  ...
  const { state } = useApp();
  const customer = state.customers.find((c) => c.id === route.params.customerId);
  ...
  const customerDocs = {
    quotations: state.quotations.filter((q) => q.customerId === customer.id),
    invoices: state.invoices.filter((inv) => inv.customerId === customer.id),
  };
      ...
      <View style={[styles.header, { backgroundColor: theme.colors.primaryContainer }]}>
        <View style={[styles.avatar, { backgroundColor: theme.colors.primary }]}>
          <Text style={styles.avatarText}>
            {customer.customerName.split(' ').map((n) => n[0]).join('').toUpperCase().slice(0, 2)}
          </Text>
        </View>
        <Text style={[styles.name, { color: theme.colors.primary }]}>{customer.customerName}</Text>
      </View>

      <View style={styles.content}>
        <View style={[styles.card, { backgroundColor: theme.colors.surface }]}>
          <Text style={[styles.sectionTitle, { color: theme.colors.onSurface }]}>Contact Details</Text>
          <DetailRow icon="call" label="Phone" value={customer.mobileNumber} theme={theme} />
          <DetailRow icon="mail" label="Email" value={customer.email} theme={theme} />
          <DetailRow icon="location" label="Address" value={customer.address} theme={theme} />
          <DetailRow icon="card" label="GST" value={customer.gstNumber} theme={theme} />
          <DetailRow icon="calendar" label="Since" value={formatDate(customer.createdAt)} theme={theme} />
        </View>
          ...
          <View style={styles.statRow}>
            <View style={[styles.stat, { backgroundColor: theme.colors.surfaceVariant }]}>
              <Text style={[styles.statValue, { color: theme.colors.primary }]}>
                {customerDocs.quotations.length}
              </Text>
        ...
        >
          ...
          <Text style={[styles.editText, { color: theme.colors.primary }]}>Edit Customer</Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  );
