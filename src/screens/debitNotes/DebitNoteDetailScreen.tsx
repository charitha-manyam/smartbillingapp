...
import { generateDebitNotePdf, sharePdf, printPdf } from '../../services/pdf';
import { debitNotePdfHtml } from '../../pdf/templates';

export default function DebitNoteDetailScreen({ route }: any) {
  const theme = useTheme();
  const { state } = useApp();
  const [busy, setBusy] = useState(false);
  const note = state.debitNotes.find((dn) => dn.id === route.params.debitNoteId);

  if (!note) {
    return (
      <View style={[styles.container, styles.center, { backgroundColor: theme.colors.background }]}>
        <Text>Debit Note not found</Text>
      </View>
    );
  }
  ...
  const handlePrint = async () => {
    try {
      setBusy(true);
      await printPdf(debitNotePdfHtml(note, state.company));
    } catch {
  ...
  const handleShare = async () => {
    try {
      setBusy(true);
      const uri = await generateDebitNotePdf(note, state.company);
      await sharePdf(uri);
    } catch {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <View style={[styles.header, { backgroundColor: '#EF4444' }]}>
        <Text style={styles.docNumber}>{note.debitNoteNumber}</Text>
      ...
      <View style={styles.section}>
        ...
        <Text style={[styles.value, { color: theme.colors.onSurface }]}>{note.vendorName}</Text>
      </View>
